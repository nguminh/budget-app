...
create unique index if not exists categories_default_unique_name_kind
  on public.categories (lower(name), kind)
  where user_id is null;

create unique index if not exists categories_user_unique_name_kind
...
drop policy if exists "categories_insert_own_only" on public.categories;
...
drop policy if exists "categories_update_own_only" on public.categories;
...
drop policy if exists "categories_delete_own_only" on public.categories;
